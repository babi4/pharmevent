  ...
  def create
    ...
    respond_to do |format|
      if @documents_nal_rashod.save
        format.html { redirect_to root_or_current_path(@event) , notice: 'Наличный расход создан.' }
  ...
  def update
    respond_to do |format|
      if @documents_nal_rashod.update_attributes(params[:documents_nal_rashod])
        format.html { redirect_to root_or_current_path(@event), notice: 'Наличный расход изменен.' }
        format.json { head :no_content }
      else
