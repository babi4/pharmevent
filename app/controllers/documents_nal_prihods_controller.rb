  ...
  def create
    ...
    respond_to do |format|
      if @documents_nal_prihod.save
        format.html { redirect_to root_or_current_path(@event), notice: 'Наличный приход создан.' }
  ...
  def update
    respond_to do |format|
      if @documents_nal_prihod.update_attributes(params[:documents_nal_prihod])
        format.html { redirect_to root_or_current_path(@event), notice: 'Наличный приход изменен.' }
        format.json { head :no_content }
      else
