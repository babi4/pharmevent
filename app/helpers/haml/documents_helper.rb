...
module Haml::DocumentsHelper
  def links_collection(event, document)
    ...
    if can? :revise, document
      haml_tag :li, link_to( 'Исправлено', send( "update_state_event_#{document_type}_path", event, document, transaction: "revise" ), method: :put )
    end
    ...
    if can? :receive, document
      haml_tag :li, link_to( 'Документы получены', send( "update_state_event_#{document_type}_path", event, document, transaction: "receive" ), method: :put )
    end
    ...
    if can? :add_to_1c, document
      haml_tag :li, link_to( 'Внесено в 1С', send( "update_state_event_#{document_type}_path", event, document, transaction: "add_to_1c" ), method: :put )
    end

    if can? :set_ready_to_post, document
      haml_tag :li, link_to( 'Документы готовы', send( "update_state_event_#{document_type}_path", event, document, transaction: "set_ready_to_post" ), method: :put )
    end

    if can? :post, document
      haml_tag :li, link_to( 'Документы отправлены', send( "update_state_event_#{document_type}_path", event, document, transaction: "post" ), method: :put )
    end

    if can? :complete, document
      haml_tag :li, link_to( 'Завершено', send( "update_state_event_#{document_type}_path", event, document, transaction: "complete" ), method: :put )
    end
