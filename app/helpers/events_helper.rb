...
module EventsHelper
  def event_state_text(state)
    ...
    when 'added_to_1c'
      content_tag 'span', raw('Внесен<br>в 1C'), class: 'label label-info'
    when 'ready_to_post'
      content_tag 'span', raw('Документы<br>готовы'), class: 'label label-info'
    when 'posted'
      content_tag 'span', raw('Документы<br>отправлены'), class: 'label label-info'
    when 'received'
      content_tag 'span', raw('Документы<br>получены'), class: 'label label-info'
    when 'completed'
